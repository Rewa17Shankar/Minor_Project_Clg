...
// 📌 Quiz Setup Page
router.get("/quiz-setup", ensureAuthenticated, (req, res) => {
    res.render("quiz-setup", { user: req.user, query: req.query });
});
...
// 📌 Fetch Quiz Questions
router.get("/quiz", ensureAuthenticated, async (req, res) => {
    const { category, difficulty } = req.query;
    if (!category || !difficulty) {
        return res.redirect("/quiz-setup?error=1");
    }
    ...
    try {
        ...
        req.session.correctAnswers = response.data.results.map(q => q.correct_answer);
        req.session.quizSettings = { category, difficulty };
...
router.post("/submit", ensureAuthenticated, async (req, res) => {
    ...
    let score = 0;
    const total_questions = correctAnswers.length;
    const results = [];

    correctAnswers.forEach((correctAnswer, index) => {
        const userAnswer = userAnswers[`q${index}`] || "No Answer";
